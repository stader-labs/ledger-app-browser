...
import DappLoader from "./DappLoader";
...
export function DAPPBrowser({
  ...
  const sendMessageToDAPP: any = useCallback(
  ...
  const receiveDAPPMessage = useCallback(
    async (event: MessageEvent) => {
      if (selectedAccount && chainConfig && event.origin === dappURL.origin) {
        ...
        // if (data.method.includes("network")) {
  ...
  return (
    <Flex height="100%" flexDirection="column">
      {!!accounts.length && (
        ...
        >
          <Text color="#F7F1FF" fontSize="12px" fontWeight="600" mr="16px">
      ...
      {!!accounts.length && (
        <Center width={"100%"} display={{ base: "flex", sm: "none" }}>
          {/* <AccountRequest
            selectedAccount={selectedAccount}
